...
public static class ExecCommands
{
    ...
    private static Command CreateStepCommand(string name, string description)
    {
        ...
        cmd.SetAction(async (ParseResult pr, CancellationToken ct) =>
        {
            ...
            Environment.ExitCode = await DaemonConnector.SendCommandAsync($"exec.{name}", pJson, session);
        });
        return cmd;
    }
    ...
    private static Command CreateRunToCursor()
    {
        ...
        cmd.SetAction(async (ParseResult pr, CancellationToken ct) =>
        {
            ...
            Environment.ExitCode = await DaemonConnector.SendCommandAsync("exec.run-to-cursor", pJson, session);
        });
        return cmd;
    }
    ...
    private static Command CreatePause()
    {
        ...
        cmd.SetAction(async (ParseResult pr, CancellationToken ct) =>
        {
            ...
            var pJson = JsonSerializer.SerializeToElement(p, NdbJsonContext.Default.ExecParams);
            Environment.ExitCode = await DaemonConnector.SendCommandAsync("exec.pause", pJson, session);
        });
        return cmd;
    }
}
